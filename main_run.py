...
class SafeBatchNorm1d(nn.BatchNorm1d):
    """BatchNorm1d that avoids training-time crashes for singleton batches."""
    ...
    def set_structures(self, pathway_gene_lists: List[List[int]], adjacency: torch.Tensor):
        ...
        if self.full_graph_attention:
            ...
            if self.use_edge_mask:
                ...
                # mask = mask.masked_fill(nonedge, float('-inf'))
...
def main_5fold_cv():
    ...
    # Keep patient IDs to map indices → sample names later
    patient_ids = mut_df.index.to_numpy().astype(str) 
    ...
    all_test_y_pred = []
    all_test_y_probs = []
    all_test_pred_rows = []   # to store per-sample predictions from each fold
    ...
    for fold_data in folds:
        ...
        # Map test indices to their patient IDs
        test_sample_ids = patient_ids[test_idx]

        df_fold_preds = pd.DataFrame({
            "fold": fold,
            "sample": test_sample_ids,
            "y_label": test_metrics["y_true"],   # numpy array
            "y_score": test_metrics["y_probs"],  # probability of class 1
            "y_pred": test_metrics["y_pred"],    # predicted class
        })
        all_test_pred_rows.append(df_fold_preds)
...
# FINAL RESULTS
    ...
    # ===== NEW: Save per-sample mean prediction CSV =====
    if all_test_pred_rows:
        preds_all = pd.concat(all_test_pred_rows, ignore_index=True)
        preds_mean = (
            preds_all
            .groupby("sample", as_index=False)
            .agg(
                fold=("fold", "first"),
                y_label=("y_label", "first"),
                y_score=("y_score", "mean"),
                y_pred=("y_pred", "first"),
            )
        )
        preds_mean = preds_mean[["fold", "sample", "y_label", "y_score", "y_pred"]]
        csv_path = os.path.join(
            args.outdir,
            f"per_sample_test_predictions_mean_{graph_suffix}_{timestamp}.csv"
        )
        preds_mean.to_csv(csv_path, index=False)
        print(f"\nPer-sample mean test predictions saved to: {csv_path}")
    else:
        print("\nWarning: no per-sample predictions collected; skipping CSV.")
