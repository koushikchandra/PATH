...
def aggregate_classwise_pathway_importance_across_folds(
    outdir, n_folds=5, class1_name="Metastatic", class0_name="Primary"
):
    """
    Aggregate class-wise pathway importance across folds.

    Produces aggregate rankings for:
      1) class 1 mean weight
      2) class 0 mean weight
      3) delta = class1 - class0 (e.g., Metastatic - Primary)
    """
    pathway_dir = os.path.join(outdir, "pathway_importance")
    ...
    print(f"\n{'='*70}")
    print("AGGREGATING CLASS-WISE PATHWAY IMPORTANCE ACROSS ALL FOLDS")
    print(f"{'='*70}")

    all_fold_data = []
    for fold in range(1, n_folds + 1):
        diff_csv = os.path.join(pathway_dir, f"pathway_importance_diff_fold{fold}.csv")
        if os.path.exists(diff_csv):
            df = pd.read_csv(diff_csv)
            all_fold_data.append(df)
            print(f"Loaded fold {fold} class-wise pathway data: {len(df)} pathways")
        else:
            print(f"Warning: Missing fold {fold} class-wise data: {diff_csv}")

    if not all_fold_data:
        print("Warning: No class-wise fold data found for aggregation.")
        return None

    combined_df = pd.concat(all_fold_data, ignore_index=True)

    grouped = combined_df.groupby("pathway_idx").agg({
        "pathway_name": "first",
        "mean_weight_class1": ["mean", "std", "min", "max"],
        "mean_weight_class0": ["mean", "std", "min", "max"],
        "diff_class1_minus_class0": ["mean", "std", "min", "max"],
    }).reset_index()

    grouped.columns = [
        "pathway_idx", "pathway_name",
        "mean_class1", "std_class1", "min_class1", "max_class1",
        "mean_class0", "std_class0", "min_class0", "max_class0",
        "mean_delta", "std_delta", "min_delta", "max_delta",
    ]

    # Rank views
    class1_ranked = grouped.sort_values("mean_class1", ascending=False).reset_index(drop=True)
    class1_ranked["rank_class1"] = np.arange(1, len(class1_ranked) + 1)

    class0_ranked = grouped.sort_values("mean_class0", ascending=False).reset_index(drop=True)
    class0_ranked["rank_class0"] = np.arange(1, len(class0_ranked) + 1)

    delta_ranked = grouped.sort_values("mean_delta", ascending=False).reset_index(drop=True)
    delta_ranked["rank_delta"] = np.arange(1, len(delta_ranked) + 1)

    print(f"\nTop 20 Pathways by {class1_name} Importance:")
    print("-" * 100)
    print(f"{'Rank':<6} {'Pathway':<15} {'Name':<35} {'Mean Score':<18} {'Delta':<14}")
    print("-" * 100)
    for _, row in class1_ranked.head(20).iterrows():
        display_name = row["pathway_name"][:33] + "..." if len(row["pathway_name"]) > 35 else row["pathway_name"]
        print(
            f"{int(row['rank_class1']):<6} {int(row['pathway_idx']):<15} {display_name:<35} "
            f"{row['mean_class1']:.6f} ± {row['std_class1']:.6f} {row['mean_delta']:+.6f}"
        )

    print(f"\nTop 20 Pathways by Delta ({class1_name} - {class0_name}):")
    print("-" * 100)
    print(f"{'Rank':<6} {'Pathway':<15} {'Name':<35} {'Mean Delta':<18} {'Class1 / Class0':<20}")
    print("-" * 100)
    for _, row in delta_ranked.head(20).iterrows():
        display_name = row["pathway_name"][:33] + "..." if len(row["pathway_name"]) > 35 else row["pathway_name"]
        print(
            f"{int(row['rank_delta']):<6} {int(row['pathway_idx']):<15} {display_name:<35} "
            f"{row['mean_delta']:+.6f} ± {row['std_delta']:.6f} "
            f"{row['mean_class1']:.6f} / {row['mean_class0']:.6f}"
        )

    class1_csv = os.path.join(pathway_dir, "aggregate_pathway_importance_class1.csv")
    class0_csv = os.path.join(pathway_dir, "aggregate_pathway_importance_class0.csv")
    delta_csv = os.path.join(pathway_dir, "aggregate_pathway_importance_delta.csv")

    class1_ranked.to_csv(class1_csv, index=False)
    class0_ranked.to_csv(class0_csv, index=False)
    delta_ranked.to_csv(delta_csv, index=False)

    print(f"\nAggregate {class1_name} pathway ranking saved to: {class1_csv}")
    print(f"Aggregate {class0_name} pathway ranking saved to: {class0_csv}")
    print(f"Aggregate delta ({class1_name}-{class0_name}) ranking saved to: {delta_csv}")

    top_class1 = class1_ranked.head(20).to_dict(orient="records")
    top_delta = delta_ranked.head(20).to_dict(orient="records")
    aggregate_json = os.path.join(pathway_dir, "aggregate_classwise_pathway_importance.json")
    with open(aggregate_json, "w") as f:
        json.dump({
            "n_folds": n_folds,
            "class1_name": class1_name,
            "class0_name": class0_name,
            "delta_definition": f"{class1_name} - {class0_name}",
            "total_pathways_analyzed": int(len(grouped)),
            "top_class1_pathways": top_class1,
            "top_delta_pathways": top_delta,
            "analysis_date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        }, f, indent=2)
    print(f"Aggregate class-wise summary saved to: {aggregate_json}")
    print(f"{'='*70}\n")

    return {
        "class1_ranked": class1_ranked,
        "class0_ranked": class0_ranked,
        "delta_ranked": delta_ranked,
        "n_folds": n_folds,
    }
...
def analyze_and_save_class_pathway_importance(model, loader, pathway_names, device, outdir, fold_num):
    """
    Analyze class-conditional pathway importance and save CSVs:
      - pathway_importance_class1.csv (mean weights for class 1)
      - pathway_importance_diff.csv (mean class1 - mean class0)
    """
    model.eval()
    all_weights = []
    all_labels = []
    ...
    with torch.no_grad():
        for mut_b, cnv_b, y_b in loader:
            mut_b, cnv_b = mut_b.to(device), cnv_b.to(device)
            _, extras = model(mut_b, cnv_b, return_extras=True)
            all_weights.append(extras["pathway_weights"].cpu().numpy())
            all_labels.append(y_b.cpu().numpy())

    weights = np.vstack(all_weights)
    labels = np.concatenate(all_labels).astype(int)

    if weights.size == 0 or labels.size == 0:
        print("Warning: No data available for class-conditional pathway importance.")
        return None

    class1_mask = labels == 1
    class0_mask = labels == 0

    if not class1_mask.any() or not class0_mask.any():
        print("Warning: Missing one of the classes for class-conditional analysis.")
        return None

    mean_w1 = weights[class1_mask].mean(axis=0)
    mean_w0 = weights[class0_mask].mean(axis=0)
    diff = mean_w1 - mean_w0

    rows = []
    for idx in range(len(mean_w1)):
        pathway_name = pathway_names[idx] if idx < len(pathway_names) else f"Pathway_{idx}"
        rows.append({
            "fold": fold_num,
            "pathway_idx": int(idx),
            "pathway_name": pathway_name,
            "mean_weight_class1": float(mean_w1[idx]),
            "mean_weight_class0": float(mean_w0[idx]),
            "diff_class1_minus_class0": float(diff[idx]),
        })

    df = pd.DataFrame(rows)

    # Class 1 ranking
    df_class1 = df.sort_values("mean_weight_class1", ascending=False).reset_index(drop=True)
    df_class1["rank"] = np.arange(1, len(df_class1) + 1)
    class1_csv = os.path.join(outdir, f"pathway_importance_class1_fold{fold_num}.csv")
    df_class1.to_csv(class1_csv, index=False)
    print(f"Class 1 pathway importance saved to: {class1_csv}")

    # Differential ranking
    df_diff = df.sort_values("diff_class1_minus_class0", ascending=False).reset_index(drop=True)
    df_diff["rank"] = np.arange(1, len(df_diff) + 1)
    diff_csv = os.path.join(outdir, f"pathway_importance_diff_fold{fold_num}.csv")
    df_diff.to_csv(diff_csv, index=False)
    print(f"Differential pathway importance saved to: {diff_csv}")

    return {
        "fold": fold_num,
        "class1_csv": class1_csv,
        "diff_csv": diff_csv
    }
# ===============================
...
def main_5fold_cv():
    ...
    for fold_data in folds:
        ...
        analyze_and_save_class_pathway_importance(
            model=model,
            loader=test_loader,
            pathway_names=pathway_names,
            device=device,
...
# FINAL RESULTS
    ...
    try:
        print(f"\nAggregating class-wise pathway importance across folds...")
        aggregate_classwise_summary = aggregate_classwise_pathway_importance_across_folds(
            outdir=args.outdir,
            n_folds=5,
            class1_name="Metastatic",
            class0_name="Primary",
        )
    except Exception as e:
        print(f"Warning: Could not aggregate class-wise pathway importance: {str(e)}")
        aggregate_classwise_summary = None
    # ===== END OF ADDITION =====
    ...
    results_data = {
        ...
        'pathway_aggregation': {
            'overall_available': aggregate_pathway_summary is not None,
            'classwise_available': aggregate_classwise_summary is not None,
            'delta_definition': 'Metastatic - Primary'
        },
        'config': vars(args)
    }
