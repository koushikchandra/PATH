    ...
    def set_structures(self, pathway_gene_lists: List[List[int]], adjacency: torch.Tensor):
        ...
        if self.full_graph_attention:
            ...
            if self.use_edge_mask:
                nonedge = (A <= 0)
                # ABLATION 2: Hard mask — use float('-inf') instead of soft -10.0 penalty.
                # This disables the soft-mask mechanism for discovering new edges.
...
def main_5fold_cv():
    ...
    ap.add_argument("--outdir", default="outputs_ablation2_hard_mask")
