...
def extract_embeddings(model, loader, device):
    ...
    Returns the pooled test-set representations — one vector per patient —
    suitable for UMAP / t-SNE visualisation and silhouette scoring.
    Only call this on the test loader so that the representations are
    honest, held-out, and reflect genuine generalisation.
    """
    model.eval()
    all_embs, all_labels = [], []
...
def plot_umap_tsne_embeddings(embeddings, labels, save_dir, prefix="test_embeddings",
    ...
    """Generate UMAP and t-SNE patient visualizations for pooled test embeddings.

    Each point = one patient, coloured by class (Primary / Metastatic).
    Only test-set embeddings are used: the model never saw these during
    training, so separation reflects genuine generalisation rather than
    memorisation.  Pooling all 5 folds gives full-dataset coverage with
    zero data leakage.

    Also computes silhouette score on the raw (pre-projection) embeddings
    ...
    # Silhouette score on raw embeddings (before any dimensionality reduction)
...
def main_5fold_cv():
    ...
    for fold_data in folds:
        ...
        # Only test-set embeddings are used: honest, held-out representations
        # that reflect genuine generalisation (not training memorisation).
...
# FINAL RESULTS
    ...
    # Pooled test embedding visualisation (UMAP + t-SNE, per-patient by class)
    # Each point = one patient, coloured by Primary / Metastatic.
    # Pooling 5-fold test sets gives full-dataset coverage with zero leakage.
