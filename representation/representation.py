    ...
    def forward(self, mut: torch.Tensor, cnv: torch.Tensor, return_extras: bool = False, 
        ...
        if return_extras:
            return logits, {
                ...
                'pathway_weights': pw_weights.squeeze(-1),
...
@torch.no_grad()
def extract_patient_embeddings(model, loader, device):
    """Extract per-sample global representations used by the classifier head."""
    model.eval()
    all_emb, all_y = [], []
    for mut_b, cnv_b, y_b in loader:
        mut_b, cnv_b = mut_b.to(device), cnv_b.to(device)
        _, extras = model(mut_b, cnv_b, return_extras=True)
        all_emb.append(extras["global_repr"].detach().cpu().numpy())
        all_y.append(y_b.numpy())

    if not all_emb:
        return np.empty((0, 0), dtype=np.float32), np.array([])
    return np.vstack(all_emb), np.concatenate(all_y)


def plot_umap_tsne_embeddings(embeddings, labels, save_dir, prefix="test_embeddings", random_state=42):
    """Create UMAP and t-SNE visualizations from sample embeddings."""
    if embeddings.size == 0 or embeddings.shape[0] < 3:
        print("Warning: Not enough samples for UMAP/t-SNE visualization.")
        return {}
    ...
    try:
        ...
        for cls in np.unique(labels):
            m = labels == cls
            plt.scatter(xy[m, 0], xy[m, 1], s=20, alpha=0.8, c=palette.get(int(cls), "#555555"), label=f"Class {int(cls)}")
        plt.title("UMAP of Patient Embeddings")
        plt.xlabel("UMAP-1")
        plt.ylabel("UMAP-2")
        plt.legend()
        plt.grid(alpha=0.25)
        ...
        plt.close()
        outputs["umap_png"] = umap_path
        print(f"UMAP plot saved to: {umap_path}")
    except Exception as e:
        print(f"Warning: UMAP plot skipped ({str(e)})")
...
def main_5fold_cv():
    ...
    all_test_embeddings = []
    all_test_embedding_labels = []
    all_test_embedding_ids = []
...
#         test_metrics = evaluate_metrics(model, test_loader, device)
        ...
        # Collect per-sample embeddings for UMAP/t-SNE visualization
        fold_emb, fold_emb_labels = extract_patient_embeddings(model, test_loader, device)
        if fold_emb.size > 0:
            all_test_embeddings.append(fold_emb)
            all_test_embedding_labels.append(fold_emb_labels)
            all_test_embedding_ids.append(test_sample_ids)
...
# FINAL RESULTS
    ...
    if all_test_embeddings:
        emb_dir = os.path.join(args.outdir, "embeddings")
        os.makedirs(emb_dir, exist_ok=True)
        emb_mat = np.vstack(all_test_embeddings)
        emb_labels = np.concatenate(all_test_embedding_labels)
        emb_ids = np.concatenate(all_test_embedding_ids)

        emb_df = pd.DataFrame({"Sample": emb_ids, "label": emb_labels})
        for i in range(emb_mat.shape[1]):
            emb_df[f"emb_{i:03d}"] = emb_mat[:, i]
        emb_csv = os.path.join(emb_dir, f"5fold_test_embeddings_{graph_suffix}_{timestamp}.csv")
        emb_df.to_csv(emb_csv, index=False)
        print(f"Per-sample embeddings saved to: {emb_csv}")

        embedding_plots = plot_umap_tsne_embeddings(
            embeddings=emb_mat,
            labels=emb_labels,
            save_dir=emb_dir,
            prefix=f"5fold_test_embeddings_{graph_suffix}_{timestamp}",
            random_state=args.random_state
        )
